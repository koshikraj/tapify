...
export async function getSpendableTokenInfo(
  ...
  tokenAddress: Hex,
  account: Hex,
  validator: { address: Address; initData: Hex }
) {
  // Ethereum provider (you can use Infura or any other provider)
  ...
  const smartSession = await buildUseSmartSession(validator.address);
  ...
  return {
    limit: formatUnits(spendPolicy.spendingLimit, decimals),
    spent: formatUnits(spendPolicy.alreadySpent, decimals),
    balance: formatUnits(
      spendPolicy.spendingLimit - spendPolicy.alreadySpent,
      decimals
    ),
  };
}
...
export async function getRedeemBalance(
  vaultAddress: string,
  account: string,
  provider: any
) {
...
export async function getVaultRedeemBalance(
  vaultAddress: string,
  account: string,
  provider: any
) {
